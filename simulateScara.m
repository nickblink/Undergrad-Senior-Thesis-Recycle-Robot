...
% Initialize robot
robot = ScaraInit();
pit = load('CorrectBackward60.mat')
n = pit.n;
state = zeros(6,n);  %  position on top of velocity
stateDer = zeros(6,n);
control = pit.control;
T = pit.T;
state(:,1) = pit.startState;
dt = T/(n-1);
...
%            state(:,i+1) = state(:,i) + dt*(stateDer(:,i)); %+ stateDer(:,i+1));                
        ...
        statePath = [state(1,:); state(2,:); state(3,:)]';
        stateVelocity = [state(4,:); state(5,:); state(6,:)]';
        save('CalculatedScaraBackward60','statePath','stateVelocity');    
    else   
         statePath = pit.statePath;     
    end     
    
   
    actualPath60 = load('CorrectBackward60.mat');
    actualPath60 = actualPath60.statePath;
    reCompPath60 = load('CalculatedScaraBackward60.mat');
    reCompPath60 = reCompPath60.statePath;    
    
    
    pit60actualFor = load('CorrectForward60.mat');
    actualPath60For = pit60actualFor.statePath;
    pit60reCompFor = load('CalculatedScaraForward60.mat');
    reCompPath60For = pit60reCompFor.statePath;
    ...
    hold on
     n = 60;
    plot(1:n,actualPath60For(:,1),'r',1:n,actualPath60For(:,2),'b',1:n,actualPath60For(:,3),'m',...
        1:n,reCompPath60For(:,1),'r.',1:n,reCompPath60For(:,2),'b.',1:n,reCompPath60For(:,3),'m.',...
        'MarkerSize',6)      
    h = legend('Theta1','Theta2','Distance3');
    set(h,'FontSize',textsize);
    title('Joint Positions for Forward Euler, n = 60','FontWeight','Bold','interpreter','latex','FontSize',titlesize)
    xlabel('Time','FontWeight','Bold','interpreter','latex','FontSize',titlesize)
    ylabel('Radians / Decimeters','FontWeight','Bold','interpreter','latex','FontSize',titlesize)
    ylim([-pi pi])

    text(2,-2.6,'Solid lines from fmincon','interpreter','latex','FontSize',textsize)
    text(2,-2.9,'Broken lines from Re-Integrating the control using RK22','interpreter','latex','FontSize',textsize)
    figure
    plot(1:n,actualPath60(:,1),'r',1:n,actualPath60(:,2),'b',1:n,actualPath60(:,3),'m',...
        1:n,reCompPath60(:,1),'r.',1:n,reCompPath60(:,2),'b.',1:n,reCompPath60(:,3),'m.',...
        'MarkerSize',6)      
     h = legend('Theta1','Theta2','Distance3');
    set(h,'FontSize',textsize);
    title('Joint Positions for Backward Euler, n = 60','FontWeight','Bold','interpreter','latex','FontSize',titlesize)
    xlabel('Time','FontWeight','Bold','interpreter','latex','FontSize',titlesize)
    ylabel('Radians / Decimeters','FontWeight','Bold','interpreter','latex','FontSize',titlesize)
    ylim([-pi pi])
    text(2,-2.6,'Solid lines from fmincon','interpreter','latex','FontSize',textsize)
